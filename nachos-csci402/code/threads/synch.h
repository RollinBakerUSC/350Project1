...
//Locks have two states either free or busy, used within Lock Class
typedef enum LockState {FREE, BUSY};
...
class Lock {
  ...
  private:
	...
	LockState state;		// free or busy, initialized to free
	Thread *owner;			// the thread that currently posses the lock, NULL if no owner
	List *waitQueue;		// the queue of threads waiting to obtain the lock
};
...
class Condition {
  ...
  private:
    char* name;
    // plus some other stuff you'll need to define
	Lock *waitingLock;		// the lock that is associated with this condition, starts at NULL
	List *waitQueue;		// the queue of threads waiting on this condition
};
#endif // SYNCH_H
